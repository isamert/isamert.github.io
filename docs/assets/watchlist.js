...
function drawProps(name, props) {
    return (props ?? [])
        .flatMap((prop) => [drawProp(name, prop), div(",", "comma")])
        .slice(0, -1);
}
...
function drawTodo(movie) {
    ...
    return a(
        text,
        () => draw(updateFilter({ todo: movie.todo }), SORT, MOVIES),
        {
            clazz: "movie-todo",
            title,
        },
            ...
            input("text", {
                ...
                oninput: withDebounce((e) => {
                    const filteredElems = container(
                        items
                            .filter((x) =>
                                x?.toLowerCase().includes(e.target.value),
                            )
                            .map((x) => drawProp(filter, x)),
                        "filter-popup",
                    );
...
function drawFilters(filters, sortBy) {
    ...
    sortCombo.selectedIndex = Object.values(SORTERS).findIndex(
        (x) => x === sortBy,
    );
                    ...
                    [
                        ...
                        container(
                            [
                                ...
                                movie.imdb_rating
                                    ? i({
                                          clazz: "fa fa-imdb movie-icon",
                                          title: "IMDb rating",
                                      })
                                    : "",
                                div(movie.imdb_rating, {
                                    title: "IMDb Rating",
                                }),
                                movie.metascore
                                    ? i({
                                          clazz: "fa fa-ticket movie-icon",
                                          title: "Metascore",
                                      })
                                    : "",
                        ...
                        movie.created_date
                            ? div(`Added on ${movie.created_at}`, "bottom")
                            : null,
                    ],
                    "movie-info",
                ),

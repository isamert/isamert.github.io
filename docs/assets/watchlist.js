...
function drawFilters(filters, sortBy) {
  ...
  const searchBox = input("text", {
    placeholder: "Search titles, plots, cast, genres…",
    id: "search",
    clazz: "search",
    value: SEARCH,
    oninput: withDebounce((e) => {
      const lastCursorPos = searchBox.selectionStart;
      updateSearch(e.target.value);
        draw(FILTER, SORT, MOVIES, () => {
          const search = document.getElementById("search")
          search.focus();
          search.setSelectionRange(lastCursorPos, lastCursorPos);
        });
    }, 300),
  });

  root.append(
    container([filterButtons, sortContainer, searchBox], "filter-header"),
...
function parseWatchDate(movie) {
  const value = movie.closed || movie.watched;
...
function drawTimeline(
  ...
  const count = Math.min(visibleCount, watched.length);
...
function drawMoviePoster(movie) {
  const fallback = () => container([
    div("No poster", "placeholder-label"),
    div(movie.item, "placeholder-title"),
    movie.year ? div(movie.year, "placeholder-year") : null,
  ], "movie-poster-placeholder");

  if (!movie.image) return fallback();
    ...
    container([
      ...
      container([
        ...
        container([
          ...
          movie.closed ? div(`Finished → ${movie.closed || movie.watched}`) : null,
        ], "detail-date"),
      ], "detail-info"),
    ], "detail-layout"),
  );
...
function draw(filters, sortBy, movies, then) {
  ...
  const filteredMovies = movies.filter((movie) =>
    ...
    Object.entries(filters ?? {}).every(([key, values]) =>
      values.every((value) => (movie[key] ?? []).includes(value))
    )
  );
  const filteredAndSortedMovies = filteredMovies
    .toSorted((a, b) => {
      if (sortBy) {
  ...
  const timeline = drawTimeline(filteredMovies);
  if (timeline) root.appendChild(timeline);
  root.appendChild(drawMovies(filteredAndSortedMovies));
...
function updateURLParam(key, val, replace = false) {
...
.filters-container {
  ...
  background:
    linear-gradient(
      90deg,
      transparent,
      rgb(var(--accent-rgb) / 0.055) 18%,
      rgb(var(--accent-rgb) / 0.025) 52%,
      rgb(var(--coral-rgb) / 0.035) 82%,
      transparent
    );
  border: 0;
  border-radius: 0;
  box-shadow: 0 8px 30px rgb(var(--accent-rgb) / 0.04);
}
...
.filters-container::before {
  ...
  background: linear-gradient(
    90deg,
    transparent,
    rgb(var(--accent-rgb) / 0.65) 22%,
    rgb(var(--accent-rgb) / 0.22) 72%,
    transparent
  );
}
...
.filters-container::after {
  ...
  background: linear-gradient(
    90deg,
    transparent,
    rgb(var(--coral-rgb) / 0.18) 28%,
    rgb(var(--coral-rgb) / 0.55) 78%,
    transparent
  );
}
...
.timeline-items::before {
  ...
  left: -8rem;
  height: 1px;
  background: linear-gradient(90deg, var(--accent), var(--rule) 75%, transparent);
  opacity: 0.55;
}
...
.timeline-items.expanded::before {
  display: none;
}

.timeline-items.expanded .timeline-item {
  border-top: 1px solid rgb(var(--accent-rgb) / 0.2);
}

.timeline-items.expanded .timeline-dot {
  top: -5px;
}
...
.timeline-dot {
  ...
  border-radius: 50%;
  box-shadow: 0 0 7px rgb(var(--accent-rgb) / 0.32);
...
.timeline-item:hover .timeline-movie {
  color: var(--link-hover);
}
...
.movie-card {
  ...
  cursor: pointer;
  opacity: 0;
  animation: movie-card-enter 420ms ease forwards;
  transition:
    border-color var(--transition-base),
...
.movie-card:hover {
  ...
  box-shadow:
    var(--shadow-xl),
    0 0 18px rgb(var(--accent-rgb) / 0.13),
    0 8px 24px rgb(var(--coral-rgb) / 0.055);
}
...
.movie-poster-placeholder {
  ...
  text-align: center;
  background:
    radial-gradient(circle at 25% 20%, rgb(var(--accent-rgb) / 0.12), transparent 38%),
    radial-gradient(circle at 80% 85%, rgb(var(--coral-rgb) / 0.07), transparent 42%),
    var(--bg);
}
...
.placeholder-title {
  color: var(--bright);
  font: var(--font-semibold) var(--text-base) var(--mono);
  line-height: var(--leading-tight);
}
...
.modal-content {
  ...
  padding: var(--space-2xl);
  background:
    radial-gradient(circle at 8% 0%, rgb(var(--accent-rgb) / 0.11), transparent 34%),
    radial-gradient(circle at 92% 100%, rgb(var(--coral-rgb) / 0.065), transparent 36%),
    color-mix(in srgb, var(--bg) 88%, var(--rule));
  border: 1px solid rgb(var(--accent-rgb) / 0.2);
  border-top-color: rgb(var(--accent-rgb) / 0.58);
  border-bottom-color: rgb(var(--coral-rgb) / 0.42);
  border-radius: var(--radius-lg);
  box-shadow:
    0 24px 70px color-mix(in srgb, var(--bg) 76%, transparent),
    0 0 30px rgb(var(--accent-rgb) / 0.13),
    inset 0 1px rgb(var(--accent-rgb) / 0.08);
  overflow-y: auto;
  overflow-x: hidden;
}
...
.modal-close:hover {
  ...
  background: rgb(var(--accent-rgb) / 0.13);
  border-color: rgb(var(--accent-rgb) / 0.48);
  box-shadow: 0 0 12px rgb(var(--accent-rgb) / 0.18);
...
.modal-content h2 {
  ...
  color: var(--bright);
  font: var(--font-semibold) var(--text-lg) var(--mono);
  letter-spacing: -0.02em;
  text-shadow: 0 0 12px rgb(var(--accent-rgb) / 0.12);
...
.detail-poster {
  ...
  box-shadow:
    var(--shadow-lg),
    0 0 18px rgb(var(--accent-rgb) / 0.08);
}
